...
const Depression = (props: any) => {
  const { navigation } = props

  return (
    <View style={Styles.dataWrapper}>
      <Image source={Images?.depressionMain} style={Styles.imageMain} />

      <View style={Styles.listWrapper}>
        <Text style={Styles.reviewTitle}>{`7 Reviews`}</Text>
        <TouchableOpacity onPress={() => { navigation.navigate("TherapyReview") }}>
          <Text style={Styles.gaveReview}>{`View all`}</Text>
        </TouchableOpacity>
      </View>
